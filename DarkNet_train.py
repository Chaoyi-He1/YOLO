...
def main(opt, hyp):
    ...
    linear_model = torch.nn.parallel.DistributedDataParallel(linear_model, device_ids=[opt.gpu])
    # model.yolo_layers = model.module.yolo_layers  # move yolo layer indices to top level

    # optimizer
    pg = [p for p in model.parameters() if p.requires_grad] + [p for p in linear_model.parameters() if p.requires_grad]
    ...
    for epoch in range(start_epoch, epochs + start_epoch):
        ...
        mloss, lr = train_util.train_one_epoch_for_DarkNet(model, linear_model, optimizer, train_data_loader,
                                                           device, epoch,
        ...
        if opt.notest is False or epoch == epochs - 1:
            ...
            result_info = train_util.evaluate_DarkNet(model, linear_model, val_data_loader, device) / 10000

            # only first process in DDP process to record info and save weights
            if opt.rank in [-1, 0]:
                print("Test acc: ", result_info)

                # update best mAP(IoU=0.50:0.95)
                if result_info > best_acc:
                    best_acc = result_info
                ...
                if opt.savebest is False:
                    ...
                    with open(results_file, 'r') as f:
                        ...
                        torch.save(save_files, "./weights/DarkNet-{}.pt".format(epoch))
                else:
                    # only save best weights
                    if best_acc == result_info:
                        with open(results_file, 'r') as f:
                            save_files = {
                                'model': model.module.state_dict(),
                                'optimizer': optimizer.state_dict(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='cfg/Darknet_net.cfg', help="*.cfg path")
