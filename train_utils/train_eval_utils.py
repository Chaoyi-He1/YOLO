...
def train_one_epoch_for_DarkNet(model, linear_model, optimizer, data_loader, device, epoch,
                                print_freq, accumulate, img_size,
                                grid_min, grid_max, gs,
                                multi_scale=False, warmup=False, scaler=None):
    model.train()
    ...
    mloss = torch.zeros(2).to(device)  # mean losses
    now_lr = 0.
    nb = len(data_loader)  # number of batches
    # imgs: [batch_size, 3, img_size, img_size]
    # targets: [num_obj, 6] , that number 6 means -> (img_index, obj_index, x, y, w, h)
    ...
    for i, (imgs, targets, paths, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with amp.autocast(enabled=scaler is not None):
            pred = model(imgs)
            pred = linear_model(pred)

            # loss
            loss_dict = compute_loss_for_DarkNet(pred, targets, model)
            losses = sum(loss for loss in loss_dict.values())

        # reduce losses over all GPUs for logging purpose
        loss_dict_reduced = utils.reduce_dict(loss_dict)
        losses_reduced = sum(loss for loss in loss_dict_reduced.values())
        loss_items = torch.cat((loss_dict_reduced["class_loss"],
                                losses_reduced)).detach()
        mloss = (mloss * i + loss_items) / (i + 1)  # update mean losses
...
@torch.no_grad()
def evaluate_DarkNet(model, linear_model, data_loader, device=None):
    cpu_device = torch.device("cpu")
    model.eval()
    linear_model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
