...
def compute_loss_for_DarkNet(p, targets, model):  # predictions, targets, model
    device = p[0].device
    lcls = torch.zeros(1, device=device, dtype=torch.float)  # Tensor(0)
    h = model.hyp  # hyperparameters
    red = 'mean'  # Loss reduction (sum or mean)
    t = targets[:, 1].view(-1).type(torch.LongTensor).to(device)

    # Define criteria
    CEcls = nn.CrossEntropyLoss().to(device)
    lcls += CEcls(p, t)
    return {"class_loss": lcls}
